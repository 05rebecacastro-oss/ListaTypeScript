...
class Chamado{
    ...
    finalizarChamado(concluido:boolean):void{
        if (conclusao == 1){
            concluido == true 
        }else {
            concluido == false
...
for(let i=0; i<quantidade; i++){
    ...
    let concluido:number = Number(prompt("a quantidade que vc queria foi cadastrada (1-sim,2-nao)?"))

    let novoChamado:Chamado = new Chamado(id, descricaoEquipamento,laboratorio,concluido)
        novoChamado.finalizarChamado(concluido)
